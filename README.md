...
### Важные замечания:
- **JWT_SECRET**: Обязательно измените на уникальный секретный ключ в продакшене
- **GITHUB_TOKEN**: Получите токен на GitHub (Settings → Developer settings → Personal access tokens)
- **Пароли**: Используйте надежные пароли в продакшене
- **API URLs**: При развертывании в продакшене измените на соответствующие домены
